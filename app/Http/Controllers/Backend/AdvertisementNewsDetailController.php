...
class AdvertisementNewsDetailController extends Controller
{
  public function __construct()
	...
	public function index(Request $request)
	{
		if($request->ajax())
    {
      $advertisements = Advertisement::where('section', 'news-detail')->get();
			...
			return Datatables::of($advertisements)
												...
												->editColumn('action', function($data){
													...
													$btn = '<a href="'. route('backend.news-detail-advertisement.edit', $data->id) .'" class="btn btn-primary btn-sm mr-5 edit" title="Edit">Edit</a>';
		...
		return view('backend.advertisement.advertisement-news-detail.index');
	}

	public function edit($id)
	{
		$editNewsAd = Advertisement::where('id', $id)->where('section', 'news-detail')->first();
		$images = File::allFiles(public_path('images/advertisement/'));
		return view('backend.advertisement.advertisement-news-detail.edit', compact('editNewsAd', 'images'));
	}
	...
	public function update(Request $request, $id)
	{
		...
		try{
			$categoryAd = Advertisement::where('id', $id)->where('section', 'news-detail')->first();
			...
			return redirect()->route('backend.news-detail-advertisement.index');
		}
	...
	public function destroy(Request $request, $id)
	{
		if ($request->ajax()){
			try{
				Advertisement::where('id', $id)->where('section', 'news-detail')->update(['status' => 0]);
			}
			catch(\Exception $e){
